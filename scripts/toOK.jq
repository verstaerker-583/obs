...
| (.sources[] | select(.name == "GuestA").settings.url) = "https://rtc.ninja/beta?optimize=0&r=verstaerker583mm&scene&pull=GuestA"
| (.sources[] | select(.name == "GuestB").settings.url) = "https://rtc.ninja/beta?optimize=0&r=verstaerker583mm&scene&pull=GuestB"
| (.sources[] | select(.name == "GuestX").settings.url) = "https://rtc.ninja/beta?optimize=0&r=verstaerker583mm&scene&pull=GuestX"
